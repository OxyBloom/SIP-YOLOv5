...
class ICA(nn.Module):
    ...
    def __init__(self, in_channels, reduction=32):
        ...
        # Use conv to reduce channel and produce attention
        # conv1 will take concatenated pooled features (2C from h + 2C from w) -> 4C
        self.conv1 = nn.Conv2d(in_channels * 4, self.mid, kernel_size=1, bias=False)
        self.bn = nn.BatchNorm2d(self.mid)
        self.act = nn.ReLU(inplace=True)
        self.conv_h = nn.Conv2d(self.mid, in_channels, kernel_size=1, bias=True)
        self.conv_w = nn.Conv2d(self.mid, in_channels, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        # conv1 expects 4*C input channels (x_h:2C, x_w:2C concatenated)
        y = self.conv1(y)
        y = self.bn(y)
        y = self.act(y)
